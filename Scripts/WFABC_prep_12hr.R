...
snv_2_sample <- mutate (snv_2_sample, gen_since_first_sample = 2* days_since_first_sample) # calculated generations since first sample (assume 6hr generation time)
...
for (i in 1:length(results)) {
  ...
  if (dim(ac.df)[1] != 0) {
    ...
    file_name <- paste ("Results/WFABC/", generation_name, "_names.csv", sep="")
    ...
    header_file <- paste ("Results/WFABC/", generation_name, "_header.txt", sep="")
    ...
    wfabc_file <- paste ("Results/WFABC/" , generation_name, ".txt", sep="")
...
# make empty data frame (one for depth and one for allele count)
Ne_ac.df <-data.frame(Col1 = character(),
                      Col2 = character())
Ne_ac.df <- rename (Ne_ac.df, "0" = 1, "2" =2)

Ne_dp.df <-data.frame(Col1 = character(),
                      Col2 = character())
Ne_dp.df <- rename (Ne_dp.df, "0" = 1, "2" =2)
...
for (row in 1:nrow(minor_wfabc_ac_no_names)){
  ...
  if (ncol (row_wfabc) > 1) {
    ...
    if (my_colnames[2]-my_colnames [1] ==2){
      row_wfabc <- row_wfabc %>% select ( 1,2) %>% rename ("0" = 1, "2" =2)
...
for (row in 1:nrow(minor_wfabc_DP_no_names)){
  ...
  if (ncol (row_depth) > 1) {
    ...
    if (my_colnames[2]-my_colnames [1] ==2){
      row_depth <- row_depth %>% select ( 1,2) %>% rename ("0" = 1, "2" =2)
...
Ne_file <- paste ("Results/WFABC/Ne_sample_size_input.txt", sep="")
